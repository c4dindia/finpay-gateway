...
<div class="p-2 p-lg-4 gap-4">
    <h3 class="text-center" style="Font-family:Poppins; font-weight: 500; font-size: 22px; line-height: 32px;">Companies for Paytoro Payment Services </h3>
                ...
                <div class=" d-flex justify-content-end align-items-center gap-4 lg:gap-14 mt-4 pr-3 lg:py-5">
                        ...
                        <div class="">
                            <ul class="pagination d-flex align-items-center gap-2">
                                {{-- Prev --}}
                                @if (!$p11co->onFirstPage())
                                <li class="">
                                    <a class="d-flex align-items-center" href=" {{ $p11co->previousPageUrl() ?? '#' }}">
                                ...
                                @if ($start > 1)
                                <li class=""><a class=" href=" {{ $p11co->url(1) }}" style="color: black">1</a>
                                </li>
                                @if ($start > 2)
                                ...
                                @for ($page = $start; $page <= $end; $page++)
                                    ...
                                    @else
                                    <a class="pagination-box" href="{{ $p11co->url($page) }}"
                                    ...
                                    @if ($end < $last) @if ($end < $last - 1)
                                        ...
                                        @endif
                                        <li class=""><a class="" href=" {{ $p11co->url($last) }}" style="color: black">{{ $last }}</a>
                                        </li>
                                        @endif
                                        {{-- Next --}}
                                        @if ($p11co->hasMorePages())
                                        <li class="">
                                            <a class="d-flex align-items-center" href="{{ $p11co->nextPageUrl() ?? '#' }}">
                                                <svg width="16" height="14" viewBox="0 0 16 14" fill="none" xmlns="http://www.w3.org/2000/svg">
