...
@section('title')
    P4 Services
    @php
        $currentPage = 'P4 Services';
    @endphp
@endsection

@section('css')
    <link rel="stylesheet" href="{{ asset('css/adminside/p2services.css') }}">
    <link rel="stylesheet" href="{{ asset('css/adminside/p1services.css') }}">
@endsection

@section('page-content')
    <div class="p-2 p-lg-4 gap-4">
        <h3 class="text-center" style="Font-family:Poppins; font-weight: 500; font-size: 22px; line-height: 48px;">Companies
            for StradaPay Services </h3>
        @if (count($p4co) == 0)
                        ...
                        @foreach ($p4co as $company)
                            <tr class="table-row">
                                ...
                                <td class="table-data text-center" style="border:1px solid #B8B8B8;">
                                    {{ $company->company->company_name }}</td>
                                <td class="table-data text-center" style="border:1px solid #B8B8B8;">
                                    {{ $company->company->accountId }}</td>
                ...
                <div class="d-flex justify-content-end align-items-center gap-4 lg:gap-14 mt-4 pr-3 lg:py-5">
                    ...
                    <div class="">
                        <ul class="pagination d-flex align-items-center gap-2">
                            ...
                            @if (!$p4co->onFirstPage())
                                <li class="">
                                    <a class="d-flex align-items-center" href=" {{ $p4co->previousPageUrl() ?? '#' }}">
                            ...
                            @if ($start > 1)
                                <li class=""><a class=" href=" {{ $p4co->url(1) }}" style="color: black">1</a>
                                </li>
                                @if ($start > 2)
                            ...
                            @for ($page = $start; $page <= $end; $page++)
                                <li class=" {{ $page === $current ? 'pagination-box-active' : 'pagination-box-inactive'}}">
                                    ...
                                    @else
                                        <a class="pagination-box" href="{{ $p4co->url($page) }}"
                            ...
                            @if ($end < $last) @if ($end < $last - 1)
                                ...
                                @endif
                                <li class=""><a class="" href=" {{ $p4co->url($last) }}" style="color: black">{{ $last }}</a>
                                </li>
                            @endif
                            {{-- Next --}}
                            @if ($p4co->hasMorePages())
                                <li class="">
                                    <a class="d-flex align-items-center" href="{{ $p4co->nextPageUrl() ?? '#' }}">
                                        <svg width="16" height="14" viewBox="0 0 16 14" fill="none" xmlns="http://www.w3.org/2000/svg">
