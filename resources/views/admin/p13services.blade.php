...
<div class="p-2 p-lg-4 gap-4">
    <h3 class="text-center" style="Font-family:Poppins; font-weight: 500; font-size: 22px; line-height: 32px;">Companies
        for Aliz7 Services </h3>
    <div class="d-flex flex-column flex-md-row gap-md-5 align-items-center justify-content-center">
        <h6 class="text-center mb-2 mb-md-3" style="Font-family:Poppins; font-weight: 500; line-height: 20px;">LOGIN : <a
                href="https://app.aliz7.com/" target="_blank">https://app.aliz7.com/</a></h6>
        <h6 class="text-center mb-2 mb-md-3" style="Font-family:Poppins; font-weight: 500; line-height: 20px;">( U:
            2634-001 , Pass: dsYz672GX)F# )</h6>
    </div>
    @if (count($p13co) == 0)
            ...
            <tbody style="background: white !important">
                ...
                <tr class="table-row">
                    ...
                    <td style="border:1px solid #B8B8B8; border-right:0;"
                        ...
                        @if($company->status == '1') Active @else Deactivated @endif</td>
                </tr>
                @endforeach
                @for ($i = count($p13co); $i < 15; $i++)
                    <tr class="pad">
                ...
                <div class=" d-flex justify-content-end align-items-center gap-4 lg:gap-14 mt-4 pr-3 lg:py-5">
                        ...
                        <div class="">
                            <ul class="pagination d-flex align-items-center gap-2">
                                {{-- Prev --}}
                                @if (!$p13co->onFirstPage())
                                <li class="">
                                    <a class="d-flex align-items-center" href=" {{ $p13co->previousPageUrl() ?? '#' }}">
                                ...
                                @if ($start > 1)
                                <li class=""><a class=" href=" {{ $p13co->url(1) }}" style="color: black">1</a>
                                </li>
                                @if ($start > 2)
                                ...
                                @for ($page = $start; $page <= $end; $page++) <li
                                    ...
                                    @else
                                    <a class="pagination-box" href="{{ $p13co->url($page) }}" style="color: black">{{
                                        $page }}</a>
                                    @endif
                                    </li>
                                    @endfor
                                    {{-- Trailing ellipsis + last --}}
                                    @if ($end < $last) @if ($end < $last - 1) <li class=" "><span
                                            class="rounded-[5px]">…</span></li>
                                        @endif
                                        <li class=""><a class="" href=" {{ $p13co->url($last) }}" style="color: black">{{
                                                $last }}</a>
                                        </li>
                                        @endif
                                        {{-- Next --}}
                                        @if ($p13co->hasMorePages())
                                        <li class="">
                                            <a class="d-flex align-items-center"
                                                href="{{ $p13co->nextPageUrl() ?? '#' }}">
                                                <svg width="16" height="14" viewBox="0 0 16 14" fill="none"
                                                    xmlns="http://www.w3.org/2000/svg">
                                                    <path d="M15 7L9 13M15 7L9 1M15 7L1 7" stroke="#827F7F"
