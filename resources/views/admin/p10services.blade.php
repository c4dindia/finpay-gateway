...
<div class="p-2 p-lg-4 gap-4">
    <h3 class="text-center" style="Font-family:Poppins; font-weight: 500; font-size: 22px; line-height: 32px;">Companies for Inabit Services </h3>
     <div class="d-flex flex-column flex-lg-row gap-lg-5 align-items-center justify-content-center">
        <h6 class="text-center mb-md-4" style="Font-family:Poppins; font-weight: 500; line-height: 20px;">LOGIN : <a href="https://use.inabit.com" target="_blank" class="text-break">https://use.inabit.com</a></h6>
        <h6 class="text-center mb-md-4" style="Font-family:Poppins; font-weight: 500; line-height: 20px;">( U: bobby@neurosyncventures.com , Pass: Passw0rd!100! )</h6>
    </div>
    @if (count($p10co) == 0)
        ...
        <table class="table custom-table align-middle all-trans-table">
            <thead>
                <tr>
                    ...
                    <th class="table-header text-center p-3">Cust Widget</th>
                    <th class="table-header text-center p-3">Balance</th>
                    <th class="table-header text-center p-3">Prchse Widget</th>
                    <th class="table-header text-center p-3">Balance</th>
                ...
                <div class=" d-flex justify-content-end align-items-center gap-4 lg:gap-14 mt-4 pr-3 lg:py-5">
                        ...
                        <div class="">
                            <ul class="pagination d-flex align-items-center gap-2">
                                {{-- Prev --}}
                                @if (!$p10co->onFirstPage())
                                <li class="">
                                    <a class="d-flex align-items-center" href=" {{ $p10co->previousPageUrl() ?? '#' }}">
                                ...
                                @if ($start > 1)
                                <li class=""><a class=" href=" {{ $p10co->url(1) }}" style="color: black">1</a>
                                </li>
                                @if ($start > 2)
                                ...
                                @for ($page = $start; $page <= $end; $page++)
                                    ...
                                    @else
                                    <a class="pagination-box" href="{{ $p10co->url($page) }}"
                                    ...
                                    @if ($end < $last) @if ($end < $last - 1)
                                        ...
                                        @endif
                                        <li class=""><a class="" href=" {{ $p10co->url($last) }}" style="color: black">{{ $last }}</a>
                                        </li>
                                        @endif
                                        {{-- Next --}}
                                        @if ($p10co->hasMorePages())
                                        <li class="">
                                            <a class="d-flex align-items-center" href="{{ $p10co->nextPageUrl() ?? '#' }}">
                                                <svg width="16" height="14" viewBox="0 0 16 14" fill="none" xmlns="http://www.w3.org/2000/svg">
