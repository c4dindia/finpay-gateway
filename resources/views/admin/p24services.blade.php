...
<div class="p-2 p-lg-4 gap-4">
    <h3 class="text-center" style="Font-family:Poppins; font-weight: 500; font-size: 22px; line-height: 32px;">Companies for UPI ATM Services </h3>
        ...
        <table class="table custom-table align-middle all-trans-table">
            <thead>
                <tr>
                    ...
                    <th class="table-header text-center p-3">Auth ID</th>
                        ...
                        <div class="modal-dialog modal-dialog-centered modal-lg">
                            <div class="modal-content">
                                ...
                                <div class="modal-body">
                                    <form action="{{ route('edit-p24service') }}" method="POST">
                                        @csrf
                                        ...
                                        <div class="row">
                                            <div class="col-md-6 mb-3">
                                                <label for="auth_id-{{ $company->id }}" class="form-label">Auth ID</label>
                                                <input type="text" class="form-control" id="auth_id-{{ $company->id }}" name="auth_id" value="{{ $company->auth_id }}">
                                            </div>

                                            <div class="col-md-6 mb-3">
                                                <label for="auth_key-{{ $company->id }}" class="form-label">Auth Key</label>
                                                <input type="text" class="form-control" id="auth_key-{{ $company->id }}" name="auth_key" value="{{ $company->auth_key }}">
                                            </div>
                                        </div>
                ...
                <div class=" d-flex justify-content-end align-items-center gap-4 lg:gap-14 mt-4 pr-3 lg:py-5">
                        ...
                        <div class="">
                            <ul class="pagination d-flex align-items-center gap-2">
                                {{-- Prev --}}
                                @if (!$p24co->onFirstPage())
                                <li class="">
                                    <a class="d-flex align-items-center" href=" {{ $p24co->previousPageUrl() ?? '#' }}">
                                ...
                                @if ($start > 1)
                                <li class=""><a class=" href=" {{ $p24co->url(1) }}" style="color: black">1</a>
                                </li>
                                @if ($start > 2)
                                ...
                                @for ($page = $start; $page <= $end; $page++)
                                    ...
                                    @else
                                    <a class="pagination-box" href="{{ $p24co->url($page) }}"
                                    ...
                                    @if ($end < $last) @if ($end < $last - 1)
                                        ...
                                        @endif
                                        <li class=""><a class="" href=" {{ $p24co->url($last) }}" style="color: black">{{ $last }}</a>
                                        </li>
                                        @endif
                                        {{-- Next --}}
                                        @if ($p24co->hasMorePages())
                                        <li class="">
                                            <a class="d-flex align-items-center" href="{{ $p24co->nextPageUrl() ?? '#' }}">
                                                <svg width="16" height="14" viewBox="0 0 16 14" fill="none" xmlns="http://www.w3.org/2000/svg">
