...
<div class="p-2 p-lg-4 gap-4">
    ...
    <div class="d-flex justify-content-center">
        <div class="input-group">
            ...
            <form action="{{ route('showAllTransactions') }}" method="GET" id="searchForm" class="d-flex flex-column flex-xxl-row align-items-center gap-3 gap-xxl-5 pb-md-4">
                <div class="input-group d-flex gap-3 justify-content-center">
                    ...
                    <button type="button" onclick="window.location='{{ route('showAllTransactions') }}'"
                    ...
                    <tbody class="">
                        ...
                        <tr style="cursor: pointer;">
                            ...
                            <td scope="row" class="text-nowrap" data-bs-toggle="modal"
                                data-bs-target="#transactionModal-{{ $trans->id }}">
                                {{ \App\Models\Company::where('accountId', $trans->account_id)->first()->company_name ??
                                                str_replace('/', ' ', ucfirst($trans->account_id)) }}
                            </td>
                            <td scope="row" class="limiter" data-bs-toggle="modal"
                                data-bs-target="#transactionModal-{{ $trans->id }}">
                            ...
                            <td scope="row" class="text-nowrap" data-bs-toggle="modal"
                                data-bs-target="#transactionModal-{{ $trans->id }}">
                                @if ($trans->net_amount)
                                ({{ $trans->from_currency }} {{ number_format($trans->net_amount, 2) }} +
                                {{ number_format($trans->fees, 2) }})
                                @else
                                -
                                @endif
                            </td>
                            <td scope="row" class="text-nowrap {{ $statusClasses }}">
                                ...
                                @if ($trans->status == 'p6')
                                <a href="{{ url('/p6/update/' . $trans->payment_id . '/transaction') }}"
                                    title="Refresh" class="ps-1">
                                    <i class="fa fa-refresh"></i>
                                </a>
                                @endif
                                @if ($trans->status == 'p10')
                                <a href="{{ url('/p10/update/' . $trans->payment_id . '/transaction') }}"
                                    title="Refresh" class="ps-1">
                                    <i class="fa fa-refresh"></i>
                                </a>
                                @endif
    ...
    <div class="d-flex justify-content-end align-items-center gap-4 lg:gap-14 mt-4 pr-3 lg:py-5">
        ...
        <div class="">
            <ul class="pagination d-flex align-items-center gap-2">
                ...
                @if ($start > 1)
                <li class=""><a class="" href=" {{ $transactions->url(1) }}"
                ...
                @for ($page = $start; $page <= $end; $page++)
                    ...
                    @else
                    <a class="pagination-box" href="{{ $transactions->url($page) }}"
                    ...
                    @if ($end < $last)
                        ...
                        @endif
                        <li class=""><a class="" href=" {{ $transactions->url($last) }}"
    ...
    <div class="modal-dialog modal-lg modal-dialog-centered modal-dialog-scrollable">
        <div class="modal-content">
            <div class="modal-header">
                <h1 class="modal-title fs-5" id="exampleModalLabel">Transaction Details for
                    {{ $trans->payment_id }}
                </h1>
